...
export const conversationMembers = pgTable("conversationMembers", { conversationId: varchar("conversationId", { length: 64 }).notNull(), userId: integer("userId").notNull(), joinedAt: timestamp("joinedAt").defaultNow().notNull(), lastReadAt: timestamp("lastReadAt"), role: varchar("role", { length: 16 }).default("member").notNull() }, (table) => ({ pk: primaryKey({ columns: [table.conversationId, table.userId] }) }));

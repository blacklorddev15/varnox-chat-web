...
export async function apiCall<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  // Send the session as a bearer header whenever we have it (web included). Cookies are still
  // sent as a fallback, but relying on them alone meant a dropped cookie broke every
  // authenticated call while the cached user kept the UI looking signed in.
...
export async function startPhoneVerification(phone: string, email: string): Promise<{ requestId: string; phoneMasked: string; emailMasked: string; devCode?: string; developmentMode?: boolean }> {
  return apiCall<{ requestId: string; phoneMasked: string; emailMasked: string; devCode?: string; developmentMode?: boolean }>("/api/auth/phone/start", {
    method: "POST",
...
export async function checkPhoneVerification(
  ...
  const result = await apiCall<{ app_session_id: string; user: Auth.User }>("/api/auth/phone/check", {
    method: "POST",
    body: JSON.stringify({ phone, email, requestId, code }),
  });
...
export async function loginWithPassword(username: string, password: string): Promise<{ sessionToken: string; user: Auth.User }> {
  const result = await apiCall<{ app_session_id: string; user: Auth.User }>("/api/auth/password/login", { method: "POST", body: JSON.stringify({ username, password }) });
  return { sessionToken: result.app_session_id, user: result.user };
}

export async function requestPasswordReset(email: string): Promise<{ message: string; devCode?: string }> { return apiCall("/api/auth/password/reset/start", { method: "POST", body: JSON.stringify({ email }) }); }
export async function completePasswordReset(code: string, password: string): Promise<{ success: true }> { return apiCall("/api/auth/password/reset/complete", { method: "POST", body: JSON.stringify({ code, password }) }); }
export async function startEmailVerification(username: string): Promise<{ message: string; devCode?: string }> { return apiCall("/api/auth/email/verify/start", { method: "POST", body: JSON.stringify({ username }) }); }
export async function completeEmailVerification(code: string): Promise<{ success: true }> { return apiCall("/api/auth/email/verify/complete", { method: "POST", body: JSON.stringify({ code }) }); }
export async function submitAppeal(username: string, reason: string): Promise<unknown> { return apiCall("/api/trpc/appeals.submit", { method: "POST", body: JSON.stringify({ json: { username, reason } }) }); }
